...
CREATE TABLE t_blog (
  ...
  `likes` INT(8) DEFAULT 0 NOT NULL COMMENT '喜欢',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP NULL ,
  `create_user` VARCHAR(32) NULL ,
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NULL ,
  `update_user` VARCHAR(32) NULL ,
...
CREATE TABLE t_message (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `sender` VARCHAR(255) DEFAULT '' COMMENT '留言人',
  `email` VARCHAR(255) DEFAULT '' COMMENT 'email地址',
  `message` VARCHAR(2048) COMMENT '内容',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP NULL ,
  `create_user` VARCHAR(32) NULL ,
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NULL ,
  `update_user` VARCHAR(32) NULL ,
  PRIMARY KEY (`id`)
) ENGINE=INNODB COMMENT='留言表'  DEFAULT CHARSET=utf8;

CREATE TABLE t_comment (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `commenter` VARCHAR(255) DEFAULT '' COMMENT '评论人',
  `commenter_email` VARCHAR(255) DEFAULT '' COMMENT 'email地址',
  `blog_id` INT(11) COMMENT '评论博客id',
  `comment` VARCHAR(255) DEFAULT '' COMMENT '评论内容',
  `parent_id` INT(11) COMMENT '父级评论id',
  `reply_to` VARCHAR(255) DEFAULT '' COMMENT '回复给',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP NULL ,
  `create_user` VARCHAR(32) NULL ,
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NULL ,
  `update_user` VARCHAR(32) NULL ,
  PRIMARY KEY (`id`)
) ENGINE=INNODB COMMENT='评论表'  DEFAULT CHARSET=utf8;

CREATE TABLE t_metric (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `col` VARCHAR(255) DEFAULT '' COMMENT '统计字段',
  `quota` VARCHAR(255) DEFAULT '' COMMENT '指标',
  `type` VARCHAR(255) DEFAULT ''  COMMENT '统计类型',
  `date` VARCHAR(10) DEFAULT ''  COMMENT '指标值日期',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP NULL ,
  `create_user` VARCHAR(32) NULL ,
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NULL ,
  `update_user` VARCHAR(32) NULL ,
  PRIMARY KEY (`id`)
) ENGINE=INNODB COMMENT='统计表'  DEFAULT CHARSET=utf8;
...
CREATE TABLE t_visitor (
  ...
  `update_user` VARCHAR(32) NULL ,
  PRIMARY KEY (`id`)
) ENGINE=INNODB COMMENT='访客表'  DEFAULT CHARSET=utf8;
